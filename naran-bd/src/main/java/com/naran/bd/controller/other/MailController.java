...
import com.naran.bd.param.other.MailParam;
import com.naran.bd.vo.other.MailPageVO;
import com.naran.bd.vo.other.MailVO;
...
public class MailController extends BaseController {
	...
	if (null == current) {
	    writeAjaxJSONResponse(ResultMessageBuilder.build(BdCode.GO_TO_LOGIN_CODE), response);
	    return;
	}
	...
	if (StringUtil.isBlank(param.getProvince()) || StringUtil.isBlank(param.getCity()) || StringUtil.isBlank(param.getCounty()) || StringUtil.isBlank(param.getMailAddress())) {
	    writeAjaxJSONResponse(ResultMessageBuilder.build(BdCode.FAIL_CODE, "请完善地址信息！"), response);
	    return;
	}
	if (StringUtil.isBlank(param.getMailPhone()) || StringUtil.isBlank(param.getMailName())) {
	    writeAjaxJSONResponse(ResultMessageBuilder.build(BdCode.FAIL_CODE, "请完善收件人信息！"), response);
	    return;
	}
	Mail mail = new Mail();
