...
public class AuthInterceptor extends BaseController implements HandlerInterceptor {
	...
	if (!AccountAuthCenter.authLogin(request)) {
		...
		writeAjaxJSONResponse(ResultMessageBuilder.build(BdCode.GO_TO_LOGIN_CODE, "登录过期 ！"), response);
	    } else {
		writeAjaxJSONResponse(ResultMessageBuilder.build(BdCode.GO_TO_LOGIN_CODE, "请重新登录！"), response);
